...
type SourceDte = {
  ...
  net: string; exempt: string; vat: string; total: string; details: Detail[]; references: Reference[];
  globalDiscount: string[]; tedXml: string; tedBytes: Buffer;
};
type CopySpec = { source: SourceDte; cedible: boolean; fileName: string };
type LayoutAudit = { pageCount: number; width: number; height: number; barcodeX: number; barcodeY: number; barcodeWidth: number; barcodeHeight: number; cedible: boolean; receiptBox: boolean; clippedContent: boolean; text: string };
...
function parseFinalDte(path: string, caseId: string): SourceDte {
  ...
  return {
    ...
    net: nodeText(encabezado, "MntNeto"), exempt: nodeText(encabezado, "MntExe"), vat: nodeText(encabezado, "IVA"), total: nodeText(encabezado, "MntTotal", true),
...
function documentName(type: number): string { if (type === 33) return "FACTURA ELECTRÓNICA"; if (type === 61) return "NOTA DE CRÉDITO ELECTRÓNICA"; if (type === 56) return "NOTA DE DÉBITO ELECTRÓNICA"; return fail("tipo DTE impreso no soportado"); }
...
async function barcodePng(ted: Buffer, corrupt: boolean): Promise<Buffer> {
  ...
  const output = await writeBarcode(new Uint8Array(ted), { format: "PDF417", scale: 3, options: "ecLevel=2" });
...
async function buildPdf(spec: CopySpec, options: PrintedSamplesOptions): Promise<{ bytes: Buffer; layout: LayoutAudit }> {
  ...
  pdf.setFont("helvetica", "normal"); pdf.setFontSize(8); pdf.text(`RUT: ${source.issuerRut}`, 36, 51);
  pdf.text(`GIRO: ${source.issuerActivity}`, 36, 62, { maxWidth: 350 }); pdf.text(`DIRECCIÓN: ${source.issuerAddress}, ${source.issuerCommune} ${source.issuerCity}`, 36, 75, { maxWidth: 350 });
  pdf.setDrawColor(190, 0, 0); pdf.setLineWidth(1.4); pdf.rect(405, 25, 170, 88);
  pdf.setTextColor(160, 0, 0); pdf.setFont("helvetica", "bold"); pdf.setFontSize(11); pdf.text(`RUT ${source.issuerRut}`, 420, 45);
  ...
  y = safeText(pdf, `${source.receiverName} | RUT ${source.receiverRut}`, 36, y + 11);
  ...
  if (!override?.omitReferences && source.references.length) {
    ...
    for (const ref of source.references) { y = safeText(pdf, `CodRef ${ref.code || "-"} | TpoDocRef ${ref.type} | FolioRef ${ref.folio} | FchRef ${ref.date} | RazonRef ${ref.reason}`, 36, y, 535); }
  }
  const printedTotal = override?.alterTotals ? String(Number(source.total) + 1) : source.total;
  const totalsY = Math.max(y + 7, 470); pdf.line(380, totalsY - 10, 576, totalsY - 10); pdf.text(`NETO: $${money(source.net)}`, 400, totalsY); pdf.text(`EXENTO: $${money(source.exempt)}`, 400, totalsY + 12); pdf.text(`IVA 19%: $${money(source.vat)}`, 400, totalsY + 24); pdf.setFont("helvetica", "bold"); pdf.text(`TOTAL: $${money(printedTotal)}`, 400, totalsY + 38);
  let receiptBox = false;
  if (spec.cedible && !override?.omitReceiptBox) { receiptBox = true; pdf.setFontSize(7); pdf.rect(36, 530, 335, 80); pdf.text("ACUSE DE RECIBO", 44, 542); pdf.setFont("helvetica", "normal"); pdf.text("Nombre: ____________________  RUT: ____________________", 44, 557); pdf.text("Fecha: _____________________  Firma: __________________", 44, 572); pdf.text("Recinto: ______________________________________________", 44, 587); }
  if (!spec.cedible && override?.receiptOnTaxCopy) { receiptBox = true; pdf.rect(36, 530, 335, 40); pdf.text("ACUSE DE RECIBO", 44, 545); }
  if (spec.cedible && !override?.omitCedibleLabel) { pdf.setFont("helvetica", "bold"); pdf.setFontSize(12); pdf.text("CEDIBLE", 490, 125); }
  ...
  pdf.setFontSize(9); pdf.setFont("helvetica", "bold"); pdf.text("Timbre Electrónico S.I.I.", barcodeX + 85, BARCODE_Y + BARCODE_HEIGHT + 12);
  pdf.setFont("helvetica", "normal"); pdf.setFontSize(7); pdf.text("Res. 0 de 2026 - Verifique documento en www.sii.cl", barcodeX + 55, BARCODE_Y + BARCODE_HEIGHT + 23);
  ...
  return { bytes, layout: { pageCount: pdf.getNumberOfPages(), width: PAGE_WIDTH, height: PAGE_HEIGHT, barcodeX, barcodeY: BARCODE_Y, barcodeWidth, barcodeHeight: BARCODE_HEIGHT, cedible: spec.cedible, receiptBox, clippedContent: Boolean(override?.clippedContent), text: "" } };
}

async function renderAndAudit(pdfBytes: Buffer, pngPath: string, layout: LayoutAudit, spec: CopySpec): Promise<LayoutAudit> {
  ...
  const png = Buffer.from(pixmap.asPNG()); writeFileSync(pngPath, png, { mode: 0o600 });
  ...
  const required = [documentName(spec.source.type), spec.source.folio, money(spec.source.total), "Timbre Electrónico S.I.I.", "www.sii.cl"];
  ...
  if (spec.source.references.length && !text.includes("FolioRef")) fail("referencias omitidas");
  if (spec.cedible && (!text.includes("CEDIBLE") || !layout.receiptBox || !text.includes("ACUSE DE RECIBO"))) fail("copia cedible incompleta");
