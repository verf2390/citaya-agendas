...
import { buildXmlDsigControlled, canonicalizeXmlControlled, verifyXmlSignatureControlled } from "../signing/sign-xml.real";
import type { RealXmlSigningConfig, TaxDocumentDraft, TaxDocumentReference } from "../types";
import { buildDteDocumentId, buildDteDocumentoXmlLab, buildDteSetDteXmlLab } from "../xml/build-dte-envelope";
import { escapeXml } from "../xml/escape-xml";
import { buildFacturaCertificationDocuments, type FacturaCertificationCaseId, type FacturaCertificationDocument, type FacturaCertificationReference } from "./factura-electronica-set";
import { loadFacturaPreCafInputFromPath, type FacturaPreCafInputFile } from "./pre-caf-input-loader";
...
export const FACTURA_SET_FIXTURE_OUTPUT_DIR = "/home/verf/secure/dte-lab/factura-set-4959698-dry-run";
...
const XML_DECLARATION_ISO_8859_1 = '<?xml version="1.0" encoding="ISO-8859-1"?>';
...
export type FacturaSetDryRunOptions = {
  ...
  overrides?: Partial<{
    ...
    cafRangeByCase: Partial<Record<FacturaCertificationCaseId, { from: number; to: number }>>;
    folioByCase: Partial<Record<FacturaCertificationCaseId, number>>;
    referenceSourceByCase: Partial<Record<FacturaCertificationCaseId, FacturaCertificationCaseId>>;
    ...
    realCafPath: string;
    importedCafByType: Partial<Record<33 | 56 | 61, { cafXml: string; privateKeyPem: string; publicKeyPem: string }>>;
  }>;
};
...
const sourceByNote: Partial<Record<FacturaCertificationCaseId, FacturaCertificationCaseId>> = {
  "4959698-5": "4959698-1",
  "4959698-6": "4959698-2",
  "4959698-7": "4959698-3",
  "4959698-8": "4959698-5",
...
function assertCertificationEnvironment(env: NodeJS.ProcessEnv): void {
  if (env.DTE_SII_ENV !== "certification") fail("DTE_SII_ENV debe ser certification para PRE-CAF 8");
  if (env.DTE_MODE === "production") fail("production bloqueado para PRE-CAF 8");
  if (env.DTE_CAF_PATH || env.DTE_CAF_PRIVATE_KEY_PATH) fail("rutas CAF reales bloqueadas para PRE-CAF 8");
  if (env.DTE_SII_ENABLE_SUBMIT === "true") fail("submit SII bloqueado para PRE-CAF 8");
...
function publicKeyPartsFromPrivateKey(privateKeyPem: string): { modulus: string; exponent: string } {
  const jwk = createPublicKey(createPrivateKey(privateKeyPem)).export({ format: "jwk" }) as { kty?: string; n?: string; e?: string };
  if (jwk.kty !== "RSA" || !jwk.n || !jwk.e) fail("fixture private key no contiene RSA public key");
  return {
...
function receiverKeyFor(caseId: FacturaCertificationCaseId): "receiver1" | "receiver2" | "receiver3" | "receiver4" {
...
function normalizeReference(
  ...
  const source = sourceCaseId ?? fail(`referencia especifica sin origen ${caseId}`);
  return {
...
function buildDrafts(input: FacturaPreCafInputFile, issueDate: string, taxPeriod: string, overrides: FacturaSetDryRunOptions["overrides"] = {}): TaxDocumentDraft[] {
  ...
  const sourceMap = { ...sourceByNote, ...(overrides.referenceSourceByCase ?? {}) };
  const seen = new Map<string, Set<number>>();

  return docs.map((doc) => {
    const folio = folios[doc.caseId];
    if (!Number.isSafeInteger(folio) || folio <= 0) fail(`folio fixture invalido ${doc.caseId}`);
    ...
    const receiver = input.receivers?.[receiverKeyFor(doc.caseId)] ?? fail(`receiver faltante ${doc.caseId}`);
    const usePreviousGiro = doc.caseId === "4959698-1" && input.textCorrection?.giroAnterior;
    ...
    const lines = doc.lines.map((line) => ({
      name: lineDescription ? "CORRIGE GIRO DEL RECEPTOR" : line.montoItem === 0 ? zeroAmountLineName.slice(0, 80) : line.name,
    ...
    return {
      ...
      recipient: {
        ...
        businessActivity: usePreviousGiro ? input.textCorrection?.giroAnterior : receiver.giro,
      ...
      lines,
      references: doc.references.map((reference) => normalizeReference(reference, doc.caseId, sourceMap[doc.caseId], issueDate, folios)),
      globalDiscount,
...
function assertTotals(drafts: TaxDocumentDraft[], docs: FacturaCertificationDocument[]): void {
  drafts.forEach((draft, index) => {
    if (!docs[index]) fail("documento tributario sin caso");
    if (draft.totalAmount !== (draft.netAmount ?? 0) + (draft.exemptAmount ?? 0) + (draft.taxAmount ?? 0)) fail("total DTE no cuadra");
  });
}

function assertReferences(drafts: TaxDocumentDraft[], folios: Record<FacturaCertificationCaseId, number>): void {
  ...
  drafts.forEach((draft, index) => {
    const caseId = caseByFolio.get(draft.folio) ?? fail("folio fixture sin caso");
    if (caseId !== expectedOrder[index]) fail("orden de documentos del set invalido");
    const refs = draft.references ?? [];
    if (refs[0]?.documentType !== "SET" || refs[0]?.reason !== `CASO ${caseId}` || refs[0]?.isGlobal !== true) fail("referencia SET invalida");
    if (["nota_credito", "nota_debito"].includes(draft.documentType)) {
      const expectedSource = sourceByNote[caseId] ?? fail(`origen esperado faltante ${caseId}`);
      if (
        refs.length !== 2 ||
        !refs[1].folio ||
...
function withDteNamespace(dteXml: string): string {
  return dteXml.replace("<DTE version=", `<DTE xmlns="${SII_DTE_NAMESPACE}" version=`);
}
...
function extractDocumentoForSignature(dteXml: string): string {
  ...
  return match[0].replace("<Documento ", `<Documento xmlns="${SII_DTE_NAMESPACE}" `);
}
...
function assertIso88591XmlText(xml: string): void {
  for (const char of xml) {
    ...
    const validXmlChar = code === 0x9 || code === 0xa || code === 0xd || (code >= 0x20 && code <= 0xd7ff) || (code >= 0xe000 && code <= 0xfffd);
    if (!validXmlChar) fail("caracter de control XML invalido en artefacto fixture");
...
function signAndBuildDocuments(drafts: TaxDocumentDraft[], xmlMaterial: FixtureMaterial, cafMaterial: FixtureMaterial, outputDir: string, overrides: FacturaSetDryRunOptions["overrides"] = {}): SignedDocument[] {
  return drafts.map((draft) => {
    ...
    const frmt = signFrmtControlled({ ddXml: tedWithoutFrmt.ddXml, inputEncoding: "latin1", privateKeyPem: cafPrivateKeyPem, mode: "certification" });
    ...
    const unsignedDteXml = buildDteDocumentoXmlLab(draft, { tedXml: ted.tedXml, documentSignedAt: FIXTURE_TIMESTAMP, mode: "certification", preserveTedWhitespace: true });
    ...
    const dteXml = withDteNamespace(buildDteDocumentoXmlLab(draft, { tedXml: ted.tedXml, documentSignatureXml: signatureXml, documentSignedAt: FIXTURE_TIMESTAMP, mode: "certification", preserveTedWhitespace: true }));
...
function buildEnvioXml(drafts: TaxDocumentDraft[], signedDocuments: SignedDocument[], material: FixtureMaterial, rutEnvia: string, outputDir: string): string {
  const perDocumentXml = Object.fromEntries(signedDocuments.map((doc) => [doc.draft.folio, { tedXml: doc.tedXml, documentSignatureXml: doc.signatureXml, documentSignedAt: FIXTURE_TIMESTAMP, preserveTedWhitespace: true }]));
  const setDteId = `CitayaSetDTE-4959698-FIXTURE`;
  const setDteXml = buildDteSetDteXmlLab(drafts, { setDteId, rutEnvia, perDocumentXml, mode: "certification" }).replace("<SetDTE ", `<SetDTE xmlns="${SII_DTE_NAMESPACE}" `);
  const envelopeSignature = buildXmlDsigControlled({ referenceUri: setDteId, signedXmlFragment: setDteXml, mode: "certification" }, signingConfig(material, setDteId));
  if (!envelopeSignature.verification?.ok) fail("firma XMLDSig SetDTE fixture no verifica localmente");
  const envioXml = `${XML_DECLARATION_ISO_8859_1}\n<!-- FIXTURE SIN VALIDEZ TRIBUTARIA - NO ENVIAR AL SII -->\n<EnvioDTE xmlns="${SII_DTE_NAMESPACE}" version="1.0">\n${setDteXml.replace(` xmlns="${SII_DTE_NAMESPACE}"`, "")}\n${envelopeSignature.signatureXml}\n</EnvioDTE>`;
  validateXsd(envioXml, "EnvioDTE_v10.xsd", "envio-dte-4959698-fixture", outputDir);
  if (!verifyInsertedSignature(setDteXml, envelopeSignature.signatureXml, material)) fail("firma de sobre fixture no verifica despues de insertar");
  return envioXml;
}

function assertNoPendingFolios(xml: string): void {
  if (xml.includes("PENDING_REAL_FOLIO")) fail("PENDING_REAL_FOLIO presente en XML fixture");
}
...
function writeFixtureManifest(outputDir: string, paths: string[], signedDocuments: SignedDocument[]): string {
  ...
  const manifest = {
    ...
    files: paths.map((path) => ({ file: path.split("/").pop(), sha256: sha256File(path) })),
...
function countTypes(drafts: TaxDocumentDraft[]): { type33: 4; type61: 3; type56: 1 } {
  const type33 = drafts.filter((draft) => getSiiDteTypeCode(draft.documentType) === 33).length;
  const type61 = drafts.filter((draft) => getSiiDteTypeCode(draft.documentType) === 61).length;
  const type56 = drafts.filter((draft) => getSiiDteTypeCode(draft.documentType) === 56).length;
  if (type33 !== 4 || type61 !== 3 || type56 !== 1 || drafts.length !== 8) fail("cantidad de documentos del set invalida");
  return { type33: 4, type61: 3, type56: 1 };
}

export function runFacturaSetDryRun(options: FacturaSetDryRunOptions = {}): FacturaSetDryRunResult {
  const env = options.env ?? process.env;
  const repoRoot = options.repoRoot ?? process.cwd();
  assertCertificationEnvironment(env);
  if (options.overrides?.realCafPath) fail("rutas CAF reales bloqueadas para PRE-CAF 8");

  const loadEnv = {
    ...env,
    DTE_FACTURA_PRE_CAF_ISSUE_DATE: env.DTE_FACTURA_PRE_CAF_ISSUE_DATE ?? env.DTE_CERTIFICATION_ISSUE_DATE,
  };
  const loaded = loadFacturaPreCafInputFromPath({ inputPath: env.DTE_FACTURA_PRE_CAF_INPUT_PATH, repoRoot, env: loadEnv });
  if (!loaded.ok) fail(`input PRE-CAF invalido missing=${loaded.missingFields.join(",")} invalid=${loaded.invalidFields.join(",")}`);
  const validation = validatePreCafExternalData(loaded.input);
  if (!validation.ok) fail(`contrato PRE-CAF invalido missing=${validation.missingFields.join(",")} invalid=${validation.invalidFields.join(",")}`);

  const outputDir = ensureOutputDir(options.outputDir ?? env.DTE_FACTURA_SET_DRY_RUN_OUTPUT_DIR ?? FIXTURE_OUTPUT_DIR);
  const docs = buildFacturaCertificationDocuments({
  ...
  const drafts = buildDrafts(loaded.input, loaded.issueDate, loaded.taxPeriod, options.overrides);
  if (options.overrides?.alterTotalCase) {
    const target = drafts[PRE_CAF_REQUIRED_CASE_ORDER.indexOf(options.overrides.alterTotalCase)];
    target.totalAmount += 1;
  }
  assertTotals(drafts, docs);
  const selectedFolios = { ...fixtureFolios, ...(options.overrides?.folioByCase ?? {}) };
  assertReferences(drafts, selectedFolios);
  const typeCounts = countTypes(drafts);

  const xmlMaterial = createSelfSignedFixture("citaya-pre-caf-8-xml");
  const cafMaterial = createSelfSignedFixture("citaya-pre-caf-8-caf", 768);
  const documentSigningMaterial = options.overrides?.mismatchedCertificateKey ? cafMaterial : xmlMaterial;
  try {
    const signed = signAndBuildDocuments(drafts, documentSigningMaterial, cafMaterial, outputDir, options.overrides);
    const dteSignatureOk = signed.filter((doc) => verifyInsertedSignature(doc.unsignedDocumentoXml, doc.signatureXml, xmlMaterial)).length;
    if (dteSignatureOk !== 8) fail(`firmas DTE fixture no verifican ${dteSignatureOk}/8`);
    ...
    const envioXml = buildEnvioXml(drafts, signed, xmlMaterial, loaded.input.issuer?.rutEnvia ?? "", outputDir);
    assertNoPendingFolios(envioXml);
    const writtenPaths: string[] = [];
    signed.forEach((doc) => {
      const path = join(outputDir, `${doc.caseId}-DTE-FIXTURE-SIN-VALIDEZ.xml`);
      writeSafeXml(path, doc.dteXml);
      writtenPaths.push(path);
    });
    const envioPath = join(outputDir, "EnvioDTE-4959698-FIXTURE-SIN-VALIDEZ.xml");
    writeSafeXml(envioPath, envioXml);
    writtenPaths.push(envioPath);
    const manifestPath = writeFixtureManifest(outputDir, writtenPaths, signed);
    return {
      environment: "certification",
      fixtureMode: true,
      documents: 8,
    ...
    rmSync(xmlMaterial.root, { recursive: true, force: true });
