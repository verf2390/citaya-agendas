 ...
 * Legacy presentation map; productive execution must use the DB resolver.
 * BHE automation can never be inferred from lifecycle or operational mode.
 * @returns {TenantOperationalCapabilities & { bheAutomation: false }}
 */
...
export function isSafeDemoAppointmentMode(capabilities) {
  ...
  return capabilities.lifecycleStatus === "active" &&
    ...
    capabilities.bheAutomation === false &&
...
/**
 * Appointment-scoped notifications are operational effects. Demo remains
 * fail-closed through the complete safe-demo guard; live keeps its existing
 * active/create/communication contract.
 *
 * @param {Partial<TenantOperationalCapabilities> | null | undefined} capabilities
 */
export function canRunAppointmentOperationalEffects(capabilities) {
  if (!capabilities) return false;
  if (capabilities.operationalMode === "demo") {
    return isSafeDemoAppointmentMode(capabilities);
  }
  return capabilities.operationalMode === "live" &&
    capabilities.lifecycleStatus === "active" &&
    capabilities.createAppointment === true &&
    capabilities.appointmentOperationalCommunication === true;
}
