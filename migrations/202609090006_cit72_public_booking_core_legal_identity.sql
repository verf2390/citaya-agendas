...
-- CIT-72 follow-up:
-- Public booking must require the tenant's core legal identity, not Citaya DTE
-- issuer identity. external_bhe tenants intentionally operate without Citaya DTE.
...
begin
  if not coalesce(
    (public.tenant_core_legal_gate_report(p_tenant_id)->>'identityLegalComplete')::boolean,
    false
  ) then
