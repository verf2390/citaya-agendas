...
type ConfigRow = {
  ...
  auto_email_delivery: boolean;
  tax_treatment: string;
  production_enabled: boolean;
  sii_authorization_status: string;
  ...
  readiness_tests_green: boolean;
  last_readiness_check: string | null;
...
async function loadState(tenantId: string, authMode: string) {
  ...
  const [
    ...
    supabaseAdmin
      ...
      .select("dte_type,active")
      .eq("tenant_id", tenantId),
    supabaseAdmin
      .from("dte_production_folio_ledger")
      .select("dte_type,state")
      .eq("tenant_id", tenantId),
    loadAdminDocumentRows(tenantId),
    supabaseAdmin
      .from("dte_tenant_readiness_evidence")
      .select(
        "trust_anchor_valid,trust_anchor_sha256,trust_anchor_acquisition_ready,caf_import_fail_closed",
      )
      .eq("tenant_id", tenantId)
      .maybeSingle(),
    supabaseAdmin
      ...
      .select(
        "issuer_profile_state,enabled,issuer_rut,issuer_legal_name,issuer_activity,issuer_address,issuer_commune,issuer_city",
      )
      .eq("tenant_id", tenantId)
      .maybeSingle(),
    supabaseAdmin
      ...
      .select(
        "authorization_date,authorized_types,evidence_source,evidence_fingerprint,registered_at,status",
      )
      .eq("tenant_id", tenantId)
      .eq("status", "current")
      .maybeSingle(),
  ...
  const readinessEvidence = (readinessEvidenceResult.data ??
    {}) as Partial<ReadinessEvidenceRow>;
  const issuerProfile = (issuerProfileResult.data ??
  ...
  const taxDataReady = Boolean(
    issuerProfile.issuer_legal_name &&
    issuerProfile.issuer_rut &&
    issuerProfile.issuer_activity &&
    issuerProfile.issuer_address &&
  ...
  const cafFoliosReady = Boolean(
    config.caf_ready &&
    config.folio_ready &&
    cafs.some((row) => row.active) &&
    folios.some((row) => row.state === "available"),
  );
  const authorizedTypes = Array.isArray(
    authorizationResult.data?.authorized_types,
  )
  ...
  const compliance = deriveBillingCompliance({
    ...
    authorizationEvidenceCurrent:
      authorizationResult.data?.status === "current",
    authorizedTypes,
    activeTypes,
    activationGates: {
      33: gate33Result.data as BillingActivationGate,
      39: gate39Result.data as BillingActivationGate,
  ...
  const steps = [
    {
      ...
      ready: taxDataReady,
      detail: taxDataReady
        ? "Identidad tributaria completa."
        : "Completa RUT, razón social, giro y dirección.",
      action: "Editar datos",
    },
    {
      key: "certificate",
      ...
      detail: certificateReady
        ? "Certificado vigente según evidencia persistida."
        : "Carga y valida un certificado vigente.",
      action: "Revisar certificado",
    },
    {
      key: "caf",
      label: "CAF y folios",
      ready: cafFoliosReady,
      detail: cafFoliosReady
        ? "Hay CAF activo y folios disponibles."
        : "Importa CAF oficial y verifica folios disponibles.",
      action: "Revisar folios",
    },
    {
      key: "authorization",
      label: "Autorización SII",
      ready: authorizationReady,
      detail: authorizationReady
        ? "Aprobación persistida para este tenant."
        : "No existe evidencia persistida de aprobación para este tenant.",
      action: "Revisar autorización",
    },
    {
      key: "automation",
      label: "Automatización",
      ready: automationReady,
      detail: automationReady
        ? "Storage, endpoints, worker y pruebas están listos."
        : "Completa storage, endpoints, worker y pruebas de readiness.",
  ...
  for (const row of folios) {
    const current = byType.get(row.dte_type) ?? {
      available: 0,
      reserved: 0,
      issued: 0,
    };
  ...
  return {
    ...
    status: {
      ...
      blockingReason: compliance.readyForFirstInvoiceFromUi
        ? null
        : (config.safe_blocking_reason ??
          (!globalEnabled ? "Activación legal global pendiente." : null)),
    },
    steps,
    policy: {
      ...
      effectiveAutomatic:
        compliance.issuanceEnabled &&
        config.issuance_mode === "automatic_on_verified_payment",
      depositTaxDocumentPolicyStatus:
        config.deposit_tax_document_policy_status ?? "unconfigured",
      boletaPaymentDocumentModel:
        config.boleta_payment_document_model ?? "unconfigured",
      boletaModelVerifiedAt: config.boleta_model_verified_at ?? null,
      boletaModelVerifiedBy: config.boleta_model_verified_by ?? null,
      boletaModelEvidenceReference:
        config.boleta_model_evidence_reference ?? null,
      paymentMethodTaxPolicies: Object.fromEntries(
        ((methodTaxPoliciesResult.data ?? []) as MethodTaxPolicyRow[]).map(
          (row) => [row.provider, row.classification],
        ),
      ),
    },
    tax: {
      ...
      businessActivity:
        issuerProfile.issuer_activity ?? billing.business_activity ?? "",
    ...
    readiness: {
      ...
      siiAuthorizationStatus:
        config.sii_authorization_status ?? "not_configured",
      certificateReady,
      cafReady: cafFoliosReady,
      lastCheck: config.last_readiness_check ?? null,
      folios: Object.fromEntries(
        [...byType.entries()].map(([type, counts]) => [String(type), counts]),
      ),
      authorizedTypes,
    ...
    declaration: {
      ...
      cafImportFailClosed: readinessEvidence.caf_import_fail_closed === true,
      productionCafCount: cafs.filter((row) => row.active).length,
      availableFolioCount: folios.filter((row) => row.state === "available")
        .length,
    },
...
export async function GET(req: Request) {
  ...
  try {
    await assertTenantCanAdministerTax(auth.tenantId);
  } catch {
    return NextResponse.json(
      { ok: false, error: "Configuración no disponible para este entorno" },
      { status: 409 },
    );
  }
  try {
    return NextResponse.json({
      ok: true,
      state: await loadState(auth.tenantId, auth.authMode),
    });
  } catch {
    return NextResponse.json(
      {
        ok: false,
        error: "No se pudo cargar el estado tributario del negocio.",
      },
      { status: 503 },
    );
  }
...
export async function PATCH(req: Request) {
  ...
  const issuanceMode =
    body.issuanceMode === "automatic_on_verified_payment"
      ? body.issuanceMode
      : "manual";
  const consumerDocumentType = ["39", "41", "unsupported"].includes(
    body.consumerDocumentType,
  )
    ? body.consumerDocumentType
    : "unsupported";
  const taxTreatment = ["affected", "exempt", "mixed", "unconfigured"].includes(
    body.taxTreatment,
  )
    ? body.taxTreatment
    : "unconfigured";
  const depositTaxDocumentPolicyStatus = [
    "unconfigured",
    "reviewed",
    "enabled",
  ].includes(body.depositTaxDocumentPolicyStatus)
    ? (body.depositTaxDocumentPolicyStatus as
        | "unconfigured"
        | "reviewed"
        | "enabled")
    : "unconfigured";
  const boletaPaymentDocumentModel = [
    ...
    : "unconfigured";
  const boletaModelEvidenceReference = text(
    body.boletaModelEvidenceReference,
    500,
  );
  if (
    boletaPaymentDocumentModel !== "unconfigured" &&
    boletaModelEvidenceReference.length < 3
  ) {
    return NextResponse.json(
      {
        ok: false,
        error:
          "Registra una referencia administrativa de la verificación del modelo de boleta.",
      },
      { status: 400 },
    );
  }
  const configPayload = {
    ...
    boleta_model_verified_by:
      boletaPaymentDocumentModel === "unconfigured" ? null : auth.userId,
    boleta_model_evidence_reference:
      boletaPaymentDocumentModel === "unconfigured"
        ? null
        : boletaModelEvidenceReference,
  ...
  const configResult = await supabaseAdmin
    .from("dte_tenant_issuance_settings")
    .upsert(configPayload, { onConflict: "tenant_id" });
  if (configResult.error)
    return NextResponse.json(
      { ok: false, error: "No se pudo guardar la política de emisión." },
      { status: 500 },
    );

  const methodPolicies =
    boletaPaymentDocumentModel === "electronic_payment_voucher_as_boleta" &&
    body.paymentMethodTaxPolicies &&
    typeof body.paymentMethodTaxPolicies === "object"
      ? (body.paymentMethodTaxPolicies as Record<string, unknown>)
      : {};
  const methodPolicyResults = await Promise.all(
    (["mercadopago", "webpay", "khipu", "manual"] as const).map(
      async (provider) => {
        const classification = methodPolicies[provider];
        if (
          !["voucher_as_boleta", "requires_boleta"].includes(
    ...
    return NextResponse.json(
      {
        ok: false,
        error:
          "No se pudo guardar la clasificación tributaria de los medios de pago.",
      },
      { status: 500 },
    );
  }
  ...
  if (body.tax && typeof body.tax === "object") {
    ...
    const billingPayload = {
      tenant_id: auth.tenantId,
      legal_name: text(tax.legalName),
      tax_id: text(tax.taxId, 32),
      business_activity: text(tax.businessActivity),
      tax_address: text(tax.address),
      tax_commune: text(tax.commune, 100),
      tax_city: text(tax.city, 100),
      tax_email: text(tax.email, 254),
      tax_phone: text(tax.phone, 32),
      updated_at: new Date().toISOString(),
    };
    const billingResult = await supabaseAdmin
      .from("tenant_billing_settings")
      .upsert(billingPayload, { onConflict: "tenant_id" });
    if (billingResult.error)
      return NextResponse.json(
        { ok: false, error: "No se pudieron guardar los datos tributarios." },
        { status: 500 },
      );
    ...
    if (issuerResult.error) {
      return NextResponse.json(
        {
          ok: false,
          error: "No se pudo actualizar la identidad tributaria maestra.",
        },
        { status: 500 },
      );
    }
