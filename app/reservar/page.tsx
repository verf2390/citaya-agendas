...
function ReservarInner() {
  ...
  // 3) ✅ Cargar servicios por tenant (SERVER-SIDE)
  useEffect(() => {
    if (!tenantSlug) return;
    ...
    (async () => {
      try {
        ...
        // Se espera: { services: [...] }
        const list = (Array.isArray(json?.services) ? json.services : []) as Service[];
        ...
        if (!cancelled) {
          setServices([]);
          setService(null);
          setLoadError((prev) => prev ?? (e?.message ?? "No se pudieron cargar servicios."));
        }
      } finally {
