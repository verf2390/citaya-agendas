      ...
      <div className="mx-auto w-full max-w-6xl">
        ...
        <div className="mt-10 grid grid-cols-1 gap-5 sm:mt-12 sm:grid-cols-2 sm:gap-6 lg:grid-cols-3 lg:gap-7">
          {services.map((service) => (
            <article
              key={service.title}
              className="rounded-2xl border border-slate-300 bg-white p-5 shadow-[0_24px_55px_-30px_rgba(15,23,42,0.48)] transition-all duration-300 hover:-translate-y-2.5 hover:border-cyan-400 hover:shadow-[0_36px_78px_-26px_rgba(8,145,178,0.55)] sm:p-6 [&:nth-child(2n)]:bg-slate-50/70"
            >
              <h3 className="text-lg font-semibold text-slate-900">{service.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-slate-700 sm:text-base">{service.description}</p>
            </article>
          ))}
        </div>
      </div>
