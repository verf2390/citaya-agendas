...
const quickQuoteWhatsappMessage = encodeURIComponent(
  "Hola Victor, vi la demo y quiero algo así para mi negocio de servicios.",
);
      ...
      <RevealOnScroll className="mx-auto grid max-w-6xl items-center gap-8 rounded-[2rem] border border-indigo-100/80 bg-gradient-to-br from-indigo-50 via-white to-purple-50 p-6 shadow-[0_40px_120px_-48px_rgba(79,70,229,0.5)] sm:p-10 lg:grid-cols-[1.1fr_0.9fr] lg:p-12">
        <div>
          <span className="inline-flex rounded-full border border-indigo-200/80 bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.2em] text-indigo-700 sm:text-xs">
            Ideal para negocios que atienden por hora
          </span>

          <h1 className="mt-5 text-3xl font-bold leading-tight tracking-tight text-slate-950 sm:text-5xl lg:text-6xl">
            Haz que tus clientes reserven solos, sin perseguirte por WhatsApp
          </h1>
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-slate-600 sm:text-lg">
            Citaya convierte tus servicios, horarios y disponibilidad en una página simple donde tus clientes eligen,
            reservan y reciben confirmación automáticamente.
          </p>

          <div className="mt-7 grid gap-3 sm:mt-8 sm:flex sm:flex-wrap">
            <Link href="https://demo.citaya.online/" target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 text-sm font-semibold text-white shadow-[0_20px_55px_-20px_rgba(79,70,229,0.85)] transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.02] hover:from-indigo-500 hover:to-purple-500 hover:shadow-[0_28px_70px_-18px_rgba(79,70,229,0.9)]">
              Ver demo funcionando
            </Link>
            <Link href={`https://wa.me/56961425029?text=${quickQuoteWhatsappMessage}`} target="_blank" rel="noreferrer" className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-emerald-300 bg-emerald-50 px-6 py-3 text-sm font-semibold text-emerald-700 transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.02] hover:bg-emerald-100 hover:shadow-[0_20px_45px_-25px_rgba(16,185,129,0.8)]">
              Quiero esto para mi negocio
            </Link>
          </div>

          <p className="mt-4 text-sm text-slate-500">Demo real · Sin compromiso · Implementación guiada</p>
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative rounded-[1.75rem] border border-slate-200/80 bg-white/90 p-4 shadow-[0_24px_80px_-35px_rgba(15,23,42,0.45)] backdrop-blur">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-indigo-600">Lo que recibes</p>
            <div className="mt-4 space-y-2 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
              <p>✅ Reserva con nombre, horario y servicio elegido</p>
              <p>✅ Confirmación automática para cada cliente</p>
              <p>✅ Vista clara de solicitudes y reservas</p>
            </div>
          </div>
        </div>
      </RevealOnScroll>
