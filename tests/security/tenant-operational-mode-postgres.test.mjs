...
const files = [
  ...
  "tests/sql/tenant-operational-mode-assertions.sql",
];

test("PostgreSQL 17 executes migrations 001-005, tenant mode RLS and triggers with rollback", () => {
  const database = `citaya_tm_${randomUUID().replaceAll("-", "")}`;
  ...
  try {
    ...
    assert.equal(verify.stdout.trim(), "t", "external transaction did not roll back migrations and fixtures");
  } finally {
